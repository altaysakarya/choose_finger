...
class _TapState extends State<Tap> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(covariant Tap oldWidget) {
    ...
    if (oldWidget.state != widget.state) {
      if (mounted) setState(() => state = widget.state);
    }
  }
  ...
  Color get color =>
      switch (state) { TapState.success => Colors.amber, _ => Colors.white };
  ...
  final double maximumSize = 150.0;
